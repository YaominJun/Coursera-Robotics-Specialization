...
n = length(x);
sample_two_movement_vertices = zeros(n,size(fv.vertices,1),size(fv.vertices,2));
sample_two_movement_faces = fv.faces;
for i = 1:n
    fv = TwoLinkRobot ([x(i), y(i)]);
    p.Vertices = fv.vertices;
    sample_two_movement(i,:,:) = fv.vertices;
    h.XData = x(i);
    h.YData = y(i);
    drawnow;
end
save('sample_two_movement_vertices');
save('sample_two_movement_faces');
save('route');